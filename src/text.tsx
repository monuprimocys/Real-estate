...
function AdvanceSerach() {
  ...
  const [selectbedroomsId, setSelectbedroomsId] = useState(() => {
    // Get initial value from localStorage or default to ""
    return localStorage.getItem("selectbedroomsId") || "";
  });
  ...
  const [selectedLocationMain, setSelectedLocationMain] = useState(
    localStorage.getItem("selectedLocationMain") || ""
  );
  const [selectedBedroom, setSelectedbedrooms] = useState(
    localStorage.getItem("selectedBedroom") || ""
  );
  ...
  const handleChangeLocationDropDwon = (event) => {
    ...
    localStorage.setItem("selectedLocationMain", mainLocation);
  };
  ...
  const handleChangebedroomsDropDwon = (event) => {
    ...
    setSelectedbedrooms(ValuesBedroom);
    localStorage.setItem("selectbedroomsId", selectedId);
    localStorage.setItem("selectedBedroom", ValuesBedroom);
  };
  ...
  const handleChangebathroomsDropDwon = (event) => {
    ...
    const selectedOption = event.target.options[event.target.selectedIndex];
    setSelectedbathrooms(selectedOption.text);
  };
  ...
  const handleChangePropertyTypeDropDwon = (event) => {
    ...
    const selectedOption = event.target.options[event.target.selectedIndex];
    setSelectedPropertyType(selectedOption.text);
  };
  ...
  const handleChangeSortbyDropDwon = (event) => {
    ...
    const selectedOption = event.target.options[event.target.selectedIndex];
    setSelectedSortby(selectedOption.text);
  };
  ...
  const handleChangeGarageDropDwon = (event) => {
    ...
    const selectedOption = event.target.options[event.target.selectedIndex];
    setSelectedGarage(selectedOption.text);
  };
    ...
    <>
          ...
          <div className="xl:w-[22%] 2xl:w-[23.2%]">
            ...
            <div className="relative">
              <select
                id="bedroom"
                className="w-full px-4 py-3 pr-10 text-gray-700 border-2 border-gray-300 rounded-lg appearance-none focus:outline-none focus:border-[#B5843F] cursor-pointer"
                value={selectbedroomsId} // The selected value is controlled by state
      ...
      <div className="w-[100%]   mx-auto overflow-hidden ">
        <AdvanceSelectedvalues
          ...
          setSelectedBedroom={setSelectedbedrooms}
...
// when code is not working properly then remove the location storage
